...
using KeysMapIterator = KeysMap::iterator;
...
protected:
	...
	// store the string to int counterpart
	// e.g. R = 82
	KeysMap mKeysMap;
...
public:
    ...
    const bool GetKey(CA_KEYS key) { return mKeys[(int)key]; }
    const bool GetKeyDown(CA_KEYS key) { return GetKeyDown((int)key); };
